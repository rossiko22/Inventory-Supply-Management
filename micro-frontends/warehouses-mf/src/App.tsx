  ...
  return (
    <div style={s.wrap}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
        <h1 style={s.h1}>Warehouses</h1>
        {isManager && <button style={{ ...s.btn, background: '#3b82f6', color: '#fff' }} onClick={openCreate}>+ New Warehouse</button>}
      </div>
      ...
      {!loading && !error && (
        <table style={s.table}>
          <thead>
            <tr>
              {['Name', 'Country', 'City', 'Capacity', 'Used', ...(isManager ? ['Actions'] : [])].map(h => (
          ...
          <tbody>
            ...
            {warehouses.length === 0 && (
              <tr><td colSpan={isManager ? 6 : 5} style={{ ...s.td, textAlign: 'center', color: '#94a3b8' }}>No warehouses found.</td></tr>
            )}
          </tbody>
        </table>
      )}
